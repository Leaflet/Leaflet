...
describe("LatLng", () => {
  ...
  describe("#distanceTo", () => {
    it("calculates distance in meters", () => {
      ...
      expect(Math.abs(Math.round(a.distanceTo(b) / 1000) - 2084) < 5).to.eql(
        true,
      );
    });
  ...
  describe("LatLng creation", () => {
    ...
    it("accepts an object with alt", () => {
      expect(new LatLng({ lat: 50, lng: 30, alt: 100 })).to.eql(
        new LatLng(50, 30, 100),
      );
      expect(new LatLng({ lat: 50, lon: 30, alt: 100 })).to.eql(
        new LatLng(50, 30, 100),
      );
    });
  });
