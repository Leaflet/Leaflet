...
describe("CRS.EPSG3857", () => {
  ...
  describe("#latLngToPoint", () => {
    ...
    it("projects the northeast corner of the world", () => {
      expect(crs.latLngToPoint(new LatLng(85.0511287798, 180), 0)).near([
        256, 0,
      ]);
    });
  });
  ...
  describe("#pointToLatLng", () => {
    it("reprojects a center point", () => {
      expect(crs.pointToLatLng(new Point(128, 128), 0)).nearLatLng(
        [0, 0],
        0.01,
      );
    });
    ...
    it("reprojects the northeast corner of the world", () => {
      expect(crs.pointToLatLng(new Point(256, 0), 0)).nearLatLng([
        85.0511287798, 180,
      ]);
    });
  });
  ...
  describe("project", () => {
    it("projects geo coords into meter coords correctly", () => {
      expect(crs.project(new LatLng(50, 30))).near([
        3339584.7238, 6446275.84102,
      ]);
  ...
  describe("unproject", () => {
    it("unprojects meter coords into geo coords correctly", () => {
      expect(crs.unproject(new Point(3339584.7238, 6446275.84102))).nearLatLng([
        50, 30,
      ]);
      expect(
        crs.unproject(new Point(20037508.34279, 20037508.34278)),
      ).nearLatLng([85.051129, 180]);
      expect(
        crs.unproject(new Point(-20037508.34279, -20037508.34278)),
      ).nearLatLng([-85.051129, -180]);
    });
...
describe("CRS.EPSG3395", () => {
  ...
  describe("#latLngToPoint", () => {
    ...
    it("projects the northeast corner of the world", () => {
      expect(crs.latLngToPoint(new LatLng(85.0840591556, 180), 0)).near([
        256, 0,
      ]);
    });
  });
...
describe("CRS.Simple", () => {
  ...
  describe("#pointToLatLng", () => {
    it("converts pixels to LatLng coords", () => {
      ...
      expect(crs.pointToLatLng(new Point(2000, 400), 1)).nearLatLng([
        -200, 1000,
      ]);
    });
  });
  ...
  describe("wrapLatLng", () => {
    it("returns coords as is", () => {
      expect(crs.wrapLatLng(new LatLng(270, 400)).equals(new LatLng(270, 400)))
        .to.be.true;
    });
