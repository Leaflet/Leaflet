...
describe("Map", () => {
  ...
  describe("#remove", () => {
    ...
    describe("corner case checking", () => {
      it("throws an exception upon reinitialization", () => {
        expect(() => new LeafletMap(container)).to.throw(
          "Map container is already initialized.",
        );
      });

      it("throws an exception if a container is not found", () => {
        expect(() => new LeafletMap("nonexistentdivelement")).to.throw(
  ...
  describe("#DOM events", () => {
    ...
    it("DOM events fired on marker can be cancelled before being caught by the map", () => {
      ...
      const layer = new Marker([1, 2]).addTo(map);
      layer
        .on("pointermove", DomEvent.stopPropagation)
        .on("pointermove", layerSpy);
      UIEventSimulator.fire("pointermove", layer._icon);
    ...
    it("pointerout is not forwarded if fired on target's child", () => {
      ...
      map.on("pointerout", mapSpy);
      layer.on("pointerout", layerSpy);
      UIEventSimulator.fire("pointerout", child, {
        relatedTarget: layer._icon,
      });
      expect(mapSpy.called).not.to.be.true;
      expect(layerSpy.called).not.to.be.true;
    });
    ...
    it("preclick is fired before click on marker and map", () => {
      let called = 0;
      const layer = new Marker([1, 2], { bubblingPointerEvents: true }).addTo(
        map,
      );
      layer.on("preclick", (e) => {
  ...
  describe("#locate", () => {
    ...
    const mockProps = Object.getOwnPropertyDescriptors(
      MockGeolocationCoordinates,
    );
    ...
    const geolocationStub = {
      geolocation: {
        getCurrentPosition(onSuccess) {
          onSuccess({
            coords: new MockGeolocationCoordinates({
              latitude: 50,
              longitude: 50,
              accuracy: 14,
            }),
