...
describe("Control.Layers", () => {
  ...
  describe("baselayerchange event", () => {
    it("is fired on input that changes the base layer", () => {
      const baseLayers = {
          "Layer 1": new TileLayer(""),
          "Layer 2": new TileLayer(""),
        },
        layers = new Control.Layers(baseLayers).addTo(map),
        spy = sinon.spy();

      map.on("baselayerchange", spy);
      UIEventSimulator.fire(
        "click",
        layers._baseLayersList.getElementsByTagName("input")[0],
      );
      ...
      UIEventSimulator.fire(
        "click",
        layers._baseLayersList.getElementsByTagName("input")[1],
      );
    ...
    it("is not fired on input that doesn't change the base layer", () => {
      const overlays = {
          "Marker 1": new Marker([0, 0]),
          "Marker 2": new Marker([0, 0]),
        },
      ...
      UIEventSimulator.fire(
        "click",
        layers._overlaysList.getElementsByTagName("input")[0],
      );
  ...
  describe("updates", () => {
    ...
    it("when an included layer is added or removed from the map, it's (un)checked", () => {
      const baseLayer = new TileLayer(),
        overlay = new Marker([0, 0]);
      new Control.Layers({ Baselayer: baseLayer }, { Overlay: overlay }).addTo(
        map,
      );

      function isChecked() {
        return !!map._container.querySelector(
    ...
    it("updates when an included layer is removed from the control", () => {
      ...
      expect(
        map._container.querySelector(".leaflet-control-layers-overlays")
          .children.length,
      ).to.be.equal(0);
    });
    ...
    it("having repeated layers works as expected", () => {
      ...
      function checkInputs(idx) {
        const inputs = map._container.querySelectorAll(
      ...
      UIEventSimulator.fire(
        "click",
        layers._baseLayersList.getElementsByTagName("input")[2],
      );
      checkInputs(2);
      expect(map._layers[Util.stamp(layerA)]).to.be.equal(layerA);
      expect(map._layers[Util.stamp(layerB)]).to.be.equal(undefined);
  ...
  describe("is created with an expand link", () => {
    it("when collapsed", () => {
      ...
      expect(map._container.querySelector(".leaflet-control-layers-toggle")).to
        .be.ok;
    });
  ...
  describe("collapse when collapsed: true", () => {
    it('expands on "Enter" keydown when toggle is focused', () => {
      const layersCtrl = new Control.Layers(null, null, {
        collapsed: true,
      }).addTo(map);
      const toggle = layersCtrl._container.querySelector(
        ".leaflet-control-layers-toggle",
      );
      UIEventSimulator.fire("keydown", toggle, { code: "Enter" });
      expect(map._container.querySelector(".leaflet-control-layers-expanded"))
        .to.be.ok;
    });
    ...
    it("expands when mouse is over", () => {
      ...
      UIEventSimulator.fire("pointerover", layersCtrl._container, {
        pointerType,
      });
      expect(map._container.querySelector(".leaflet-control-layers-expanded"))
        .to.be.ok;
    ...
    it.skipIfTouch("collapses when mouse is out", () => {
      ...
      expect(map._container.querySelector(".leaflet-control-layers-expanded"))
        .not.to.be.null;
      UIEventSimulator.fire("pointerout", layersCtrl._container, {
        pointerType,
      });
      expect(map._container.querySelector(".leaflet-control-layers-expanded"))
        .to.be.null;
  ...
  describe("does not collapse when collapsed: false", () => {
    ...
    it("is scrollable if necessary when added on map", () => {
      ...
      expect(container.clientHeight).to.be.greaterThan(
        layersCtrl._container.clientHeight,
      );
      expect(
        layersCtrl._section.classList.contains(
  ...
  it("refocus map after interaction", () => {
    ...
    UIEventSimulator.fire(
      "click",
      control._baseLayersList.getElementsByTagName("input")[0],
      {
        screenX: 0,
        screenY: 0,
      },
