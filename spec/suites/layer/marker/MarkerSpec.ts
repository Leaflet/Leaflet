describe("Marker", function () {
	...
	describe("#setIcon", function () {
		...
		it("set the correct x and y size attributes", function () {
			...
			const marker = new L.Marker([0, 0], {icon: sizedIcon});
			map.addLayer(marker);
		...
		it("changes the icon to another image while re-using the IMG element", function () {
			const marker = new L.Marker([0, 0], {icon: icon1});
			map.addLayer(marker);
		...
		it("changes the DivIcon to another DivIcon, while re-using the DIV element", function () {
			const marker = new L.Marker([0, 0], {icon: new L.DivIcon({html: 'Inner1Text'})});
			map.addLayer(marker);
	...
	describe('events', function () {
		...
		it('fires click event when clicked with DivIcon', function () {
			...
			const marker = L.marker([0, 0], {icon: new L.DivIcon()}).addTo(map);
		...
		it('fires click event when clicked on DivIcon child element', function () {
			...
			const marker = L.marker([0, 0], {icon: new L.DivIcon({html: '<img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" />'})}).addTo(map);
		...
		it("do not propagate click event", function () {
			...
			const marker = new L.Marker(new L.LatLng(55.8, 37.6));
			map.addLayer(marker);
			marker.on('click', spy);
		...
		it("do not catch event if it does not listen to it", function (done) {
			const marker = new L.Marker([55, 37]);
			map.addLayer(marker);
