...
describe("ImageOverlay", () => {
  ...
  describe("_image", () => {
    ...
    describe("className", () => {
      it("should set image's class", () => {
        expect(overlay._image.classList.contains("my-custom-image-class")).to.be
          .true;
      });
    });
  });
  ...
  describe("#getCenter", () => {
    ...
    it("should open popup at the center", () => {
      ...
      expect(overlay.getPopup().getLatLng()).to.be.nearLatLng([
        40.743078, -74.175995,
      ]);
    });
  });
  ...
  describe("crossOrigin option", () => {
    ...
    function testCrossOriginValue(crossOrigin, expectedValue) {
      it(`uses crossOrigin option value ${crossOrigin}`, () => {
        ...
        expect(overlay._image.getAttribute("crossorigin")).to.equal(
          expectedValue,
        );
      });
    }
