...
describe("Canvas", () => {
  ...
  describe("#events", () => {
    ...
    it("should be transparent for DOM events going to non-canvas features", () => {
      const marker = new Marker(map.layerPointToLatLng([150, 150])).addTo(map);
    ...
    it("should not fire click when dragging the map on top of it", (done) => {
      ...
      mouse.moveTo(50, 50, 0).down().moveBy(20, 10, 200).up();
    });

    it("does fire pointerdown on layer after dragging map", (done) => {
      // #7775
      const spy = sinon.spy();
      ...
      mouse
        ...
        .down()
        .moveBy(5, 0, 20)
        .up() // drag the map (outside of circle)
        .moveTo(300, 300, 0)
        .down()
        .moveBy(5, 0, 20)
        .up(); // expect pointerdown ok
    });
  });
