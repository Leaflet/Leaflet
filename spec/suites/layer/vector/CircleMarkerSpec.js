describe('CircleMarker', function () {
		...
		describe("when a CircleMarker is added to the map ", function () {
			describe("with a radius set as an option", function () {
				it("takes that radius", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 1}).addTo(map);
			...
			describe("and radius is set before adding it", function () {
				it("takes that radius", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 1});
					marker.setRadius(15);
					marker.addTo(map);
	...
	describe("#_opacity", function () {
		...
		describe("when a circleMarker is added to the map", function () {
			describe("with a opacity set as an option", function () {
				it("takes that opacity", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 0.5}).addTo(map);

					expect(marker._opacity).to.be(0.5);
				});
			});

			describe("and opacity is set before adding it", function () {
				it("takes that opacity", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 0.5});
					marker.setOpacity(0.7);
					marker.addTo(map);
					expect(marker._opacity).to.be(0.7);
				});
			});

			describe("and opacity is set after adding it", function () {
				it("takes that opacity", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 0.5});
					marker.addTo(map);
					marker.setOpacity(0.7);
			...
			describe("and setStyle is used to change the radius after adding", function () {
				it("takes the given opacity", function () {
					var marker = L.circleMarker([0, 0], {radius: 20, opacity: 0.5});
					marker.addTo(map);
					marker.setStyle({opacity: 0.7});
					expect(marker._opacity).to.be(0.7);
				});
			});
