...
describe("Popup", () => {
  ...
  it("sets the default 'closeButtonLabel' on the close button", () => {
    ...
    expect(
      popup.getElement().querySelector('[aria-label="Close popup"]'),
    ).not.to.equal(null);
  });
  ...
  it("sets a custom 'closeButtonLabel' on the close button", () => {
    ...
    expect(
      popup.getElement().querySelector(`[aria-label="${closeButtonLabel}"]`),
    ).not.to.equal(null);
  });
  ...
  it("can change popup content with a click on removed DOM", () => {
    const popup = new Popup()
      .setLatLng(center)
      .setContent(
        "<p onclick=\"this.parentNode.innerHTML = 'changed'\">initial</p>",
      )
      .openOn(map);
  ...
  describe("autoPan option should pan popup into visibility", () => {
    ...
    it("should not pan map to show popup content if autoPan is disabled", (done) => {
      map.on("popupopen", (e) => {
        ...
        expect(popupTopOffset).to.be.below(
          0,
          "The upper edge of the popup should not be visible",
        );
        done();
      });
    ...
    it("should pan map to show popup content if autoPan is enabled", (done) => {
      map.on("popupopen", (e) => {
        const popupTopOffset = getPopupOffset(map, e.popup);
        expect(popupTopOffset).to.equal(
          10,
          "The upper edge of the popup have a padding of 10",
        );
        done();
      });
      map.openPopup('<div style="height: 400px;"></div>', [58.4, 37.6], {
