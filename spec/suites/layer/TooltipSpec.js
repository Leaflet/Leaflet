...
describe("Tooltip", () => {
  ...
  it("opens on marker pointerover and close on pointerout", () => {
    ...
    UIEventSimulator.fire("pointerover", layer._icon, {
      relatedTarget: map._container,
    });

    expect(map.hasLayer(layer._tooltip)).to.be.true;

    UIEventSimulator.fire("pointerout", layer._icon, {
      relatedTarget: map._container,
    });
  ...
  it("opens on marker focus and closes on blur in layer group", () => {
    ...
    expect(group._tooltip._container.innerHTML).to.equal(
      "Group tooltip: Marker 1",
    );
  ...
  it("opens on marker focus and ignore layers without getElement function", () => {
    ...
    const group = new FeatureGroup([marker1, someLayerWithoutGetElement]).addTo(
      map,
    );
  ...
  it("is mentioned in aria-describedby of a bound layer", () => {
    ...
    expect(element.getAttribute("aria-describedby")).to.equal(
      tooltip._container.id,
    );
  });
  ...
  it("has class leaflet-interactive", () => {
    const layer = new Marker(center).addTo(map);
    layer.bindTooltip("Tooltip", { permanent: true, interactive: true });
    expect(layer._tooltip._container.classList.contains("leaflet-interactive"))
      .to.be.true;
  });
  ...
  it("it should use a tooltip with a function as content with a FeatureGroup", () => {
    ...
    UIEventSimulator.fire("pointerover", marker1._icon, {
      relatedTarget: map._container,
    });
    expect(map.hasLayer(group._tooltip)).to.be.true;
    ...
    UIEventSimulator.fire("pointerover", marker2._icon, {
      relatedTarget: map._container,
    });
    expect(map.hasLayer(group._tooltip)).to.be.true;
  ...
  it("opens on polygon pointerover and close on pointerout", () => {
    ...
    UIEventSimulator.fire("pointerover", layer._path, {
      relatedTarget: map._container,
    });

    expect(map.hasLayer(layer._tooltip)).to.be.true;

    UIEventSimulator.fire("pointerout", layer._path, {
      relatedTarget: map._container,
    });
    expect(map.hasLayer(layer._tooltip)).to.be.false;
  });
  ...
  it("opens a permanent tooltip and follow the pointer (sticky)", (done) => {
    ...
    layer
      .bindTooltip("Sticky", { sticky: true, permanent: true })
      .openTooltip();
    const tooltip = layer.getTooltip();
    expect(tooltip.getLatLng().equals(layer.getCenter())).to.be.true;
  ...
  it("opens the tooltip if the tooltip is loaded while the map is dragging.", () => {
    ...
    const layer = new Marker(center).bindTooltip("Tooltip", {
      permanent: true,
    });
    map.addLayer(layer);
  ...
  it("removes focus listeners after unbinding tooltip from Layer", () => {
    ...
    expect(() =>
      UIEventSimulator.fire("focus", marker.getElement()),
    ).to.not.throw();
