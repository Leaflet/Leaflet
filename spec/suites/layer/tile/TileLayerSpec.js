describe("TileLayer", () => {
	...
	describe("number of kittens loaded", () => {
		...
		it.skipIfNo3d(
			"Loads 290, unloads 275 kittens on MAD-TRD flyTo()",
			function (done) {
				...
				kittenLayer.on("load", () => {
					...
					map.on("zoomend", () => {
						...
						expect(kittenLayer._container.querySelectorAll("img").length).to.be(
							15
						);
						done();
					});
	...
	describe("url template", () => {
		...
		it("replaces {y} with y coordinate", () => {
			const layer = L.tileLayer("http://example.com/{z}/{y}/{x}.png").addTo(
				map
			);
	...
	const _describe =
    "crossOrigin" in L.DomUtil.create("img") ? describe : describe.skip; // skip in IE<11
