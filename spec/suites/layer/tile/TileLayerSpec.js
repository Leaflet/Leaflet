...
describe("TileLayer", () => {
  ...
  describe("number of kittens loaded", () => {
    ...
    it("Loads 8 kittens zoom 1", (done) => {
      kittenLayer.on("load", () => {
        ...
        expect(kittenLayer._container.querySelectorAll("img").length).to.equal(
          8,
        );
        done();
      });
    ...
    it("Loads 290, unloads 275 kittens on MAD-TRD flyTo()", function (done) {
      ...
      kittenLayer.on("load", () => {
        ...
        map.on("zoomend", () => {
          ...
          expect(
            kittenLayer._container.querySelectorAll("img").length,
          ).to.equal(15);
          done();
        });
  ...
  describe("url template", () => {
    ...
    it("replaces {y} with y coordinate", () => {
      const layer = new TileLayer("http://example.com/{z}/{y}/{x}.png").addTo(
        map,
      );
  ...
  describe("crossOrigin option", () => {
    ...
    it("resets invalid min/maxZoom to allow for tiles to be loaded without detectRetina", (done) => {
      ...
      kittenLayer.on("load", () => {
        ...
        expect(kittenLayer.options.maxZoom).to.equal(
          kittenLayer.options.minZoom,
        );
