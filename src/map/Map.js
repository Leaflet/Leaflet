...
export var Map = Evented.extend({
	...
	flyTo: (targetCenter, targetZoom, options) => {
		...
		sinh = n => { return (Math.exp(n) - Math.exp(-n)) / 2; }
		cosh =n =>  { return (Math.exp(n) + Math.exp(-n)) / 2; }
		tanh =n =>  { return sinh(n) / cosh(n); }

		var r0 = r(0);

		w= s => { return w0 * (cosh(r0) / cosh(r0 + rho * s)); }
		u= s => { return w0 * (cosh(r0) * tanh(r0 + rho * s) - sinh(r0)) / rho2; }

		easeOut =t => { return 1 - Math.pow(1 - t, 1.5); }
