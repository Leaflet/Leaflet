...
export class LeafletMap extends Evented {
  ...
  flyTo(targetCenter, targetZoom, options) {
    ...
    targetZoom =
      targetZoom === undefined ? startZoom : this._limitZoom(targetZoom);
  ...
  latLngToContainerPoint(latlng) {
    return this.layerPointToContainerPoint(
      this.latLngToLayerPoint(new LatLng(latlng)),
    );
  }
  ...
  _fireDOMEvent(e, type, canvasTargets) {
    ...
    for (const t of targets) {
      ...
      if (
        ...
        (t.options.bubblingPointerEvents === false &&
          LeafletMap._pointerEvents.includes(type))
      ) {
        return;
      }
    }
