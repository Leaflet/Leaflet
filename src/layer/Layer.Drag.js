...
L.Layer.Drag = L.Handler.extend({
  ...
  _onDown: function(event) {
    ...
    if(this._draggable || !this._layer._map) return;
    
    var defaultIcon = L.DivIcon.extend({
      options : {
    ...
    var icon = this._layer.options.dragIcon || new defaultIcon;
    
    this._marker = new L.Marker(event.latlng, {icon: icon, draggable: true});
    ...
    // simualte the original event, so we can start the drag
